...
- 栈内存
    ...
    - 在栈内存上存在一些 Java 数据（如对象引用），但 Java 对象本身的数据却是保存在堆内存的。
- 堆内存
...
- Java 没有 sizeof：Java 本身就是一种“与平台无关”的语言。
...
依次是：`public`, `protected`, （包访问权限 Package Access）没有关键字，`private`
...
Java 中的类总是继承的，要么显示地继承某个类，要么**隐式**地继承**标准根对象 `Object`**。
